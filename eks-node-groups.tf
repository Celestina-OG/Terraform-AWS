...
resource "aws_eks_node_group" "nodes_general" {
  ...
  # ARN of the IAM Role that provides permission for the EKS Node
  node_role_arn = aws_iam_role.nodes_general.arn

  # Identifiers of EC2 Subnets to associate with the EKS Node Group
  # These subnets must have the following resource tag: kubernetes.io/cluster/CLUSTER
  ...
  # Configuration block with scaling
  scaling_config {
    ...
    # Mininum number of worker nodes
    min_size = 1
  }
  ...
  # Valid values: ON_DEMAND (default), SPOT 
  capacity_type = "ON_DEMAND"

  # Disk size in GiB for worker nodes (In production 100GiB)
  disk_size = 20

  # Force version update if existing pods are unable to be drained due too pod disruption budget issue
